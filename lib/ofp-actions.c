								 ...
								 struct ofpbuf *out)
{
	...
	if((fp = fopen("/home/zhangkeyao/decode_action.log", "at")) != NULL){
		fprintf(fp, "push_sdn_tunnel: decode OK!\n");
		fprintf(fp, "src_ip: "IP_FMT"\n", IP_ARGS(tunnel->src_ip));
		fprintf(fp, "dst_ip: "IP_FMT"\n", IP_ARGS(tunnel->dst_ip));
		fprintf(fp, "id_length: %"PRIu8"\n", tunnel->id_length);
		fprintf(fp, "tunnel_id1: %"PRIu16"\n", tunnel->tun_id1);
		fprintf(fp, "tunnel_id2: %"PRIu16"\n", tunnel->tun_id2);
		fprintf(fp, "tunnel_id3: %"PRIu16"\n\n", tunnel->tun_id3);
		fclose(fp);
	}
	return 0;
}
        			   ...
        			   enum ofp_version ofp_version, struct ofpbuf *out)
{
	...
	FILE *fp;
	if((fp = fopen("/home/zhangkeyao/decode_action.log", "at")) != NULL){
			fprintf(fp, "push_sdn_tunnel: encode OK!\n");
			fprintf(fp, "src_ip: "IP_FMT"\n", IP_ARGS(oapst->src_ip));
			fprintf(fp, "dst_ip: "IP_FMT"\n", IP_ARGS(oapst->dst_ip));
			fprintf(fp, "id_length: %"PRIu8"\n", oapst->id_length);
			fprintf(fp, "tunnel_id1: %"PRIu16"\n", oapst->tun_id1);
			fprintf(fp, "tunnel_id2: %"PRIu16"\n", oapst->tun_id2);
			fprintf(fp, "tunnel_id3: %"PRIu16"\n\n", oapst->tun_id3);
...
decode_OFPAT_RAW_POP_SDN_TUNNEL(struct ofpbuf *out)
{
	...
	FILE *fp;
	if((fp = fopen("/home/zhangkeyao/decode_action.log", "at")) != NULL){
		fprintf(fp, "pop_sdn_tunnel: decode OK!\n\n");
		fclose(fp);
	}
	return 0;
}
        			   ...
        			   enum ofp_version ofp_version, struct ofpbuf *out)
{
	...
	FILE *fp;
	if((fp = fopen("/home/zhangkeyao/decode_action.log", "at")) != NULL){
			fprintf(fp, "pop_sdn_tunnel: encode OK!\n\n");
			fclose(fp);
	}
}
